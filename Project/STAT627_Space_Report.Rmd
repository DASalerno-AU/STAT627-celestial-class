...
```{r}
for (K in 1:20) {
  knn.S.pca = knn(X.S.Train.PC, X.S.Test.PC, Y.S.Train.PC, K) 
  class.rate[K] = mean(Y.S.Test.PC == knn.S.pca)
}
...
```{r}
CV.results["KNN.PC", "ValSet"] <- which.max(class.rate)
max(class.rate)
```
...
```{r}
CV.err <- (1 - CV.results)[order(CV.results$KFold, decreasing = T),]

barplot(CV.err$KFold, 
        main = "K-Fold Error Rates by Model",
        names.arg = rownames(CV.err)[1:12], 
        ylim = c(0, 0.09), 
        las = 2,
        col = "dark blue")
