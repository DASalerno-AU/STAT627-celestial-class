...
The prediction accuracy declines sharply when introducing principle components to the single pruned tree, down to 84.93%.
...
Interestingly, while K-Fold cross-validation previously improved prediction accuracy, the opposite is true concerning principle components. Prediction accuracy declines to approximately 84.82%.
...
As for LDA & QDA, the result of QDA by validation set method showed the best prediction rate, 94.91%. For the assumption of Normal distribution for LDA & QDA, we used the signed root squared ‘trans.redshift’ variable instead of the original ‘redshift’ variable. Specifically, after making the basic models and PCA (PC1,2,3) models for LDA & QDA, we implemented three cross-validation methods (LOOCV, validation set, K-fold). Among the prediction rates of the validation results, the performance of QDA was overall better than LDA’s. We may conclude that each class has its own covariance matrix, not the identical one, because LDA assumes the covariance matrix is identical.
