...
For our project, our central question was "Which statistical model would most reliably classify whether a celestial body is a star, quasar, or galaxy?"
...
# sample 5000 rows of the data for tuning, to reduce computing time
A <- sample(nrow(S.Train), 5000)
S.Tuning <- S.Train[A,]
...
for(i in 1:5){
  rf <- randomForest(class ~ u + g + r + i + z + redshift, 
                     data = S.Tuning, mtry = i)
  rf.ntrees <- which.min(rf$err.rate)
  rf <- randomForest(class ~ u + g + r + i + z + redshift, 
                     data = S.Tuning, mtry = i, ntree = rf.ntrees)
...
The random forest method achieves the highest prediction accuracy of all tree-based models at 97.74%. 

```{r VS random forest PCA}
set.seed(1)

# sample 5000 rows of the data for tuning
B <- sample(nrow(S.Train), 5000)
S.Tuning.PC <- S.Train.PC[B,]
...
for(i in 1:3){
  rf <- randomForest(class ~ PC1 + PC2 + PC3, 
                     data = S.Tuning.PC, mtry = i)
  rf.ntrees <- which.min(rf$err.rate)
  rf <- randomForest(class ~ PC1 + PC2 + PC3, 
                     data = S.Tuning.PC, mtry = i, ntree = rf.ntrees)
  y_hat <- predict(rf, newdata = S.Test.PC)
  rf.err[i] <- mean(y_hat != Y.S.Test)
  ntrees[i] <- rf.ntrees
}
...
y_hat <- predict(rf.optimal, newdata = S.Test.PC, type = 'class')
table(y_hat, Y.S.Test)
CV.results["RF.PC", "ValSet"] <- mean(y_hat == Y.S.Test)
```

Remaining consistent with prior models, principal components in the random forest model decrease prediction accuracy to 96.84%.
...
The model with the best prediction rate was the random forest model at 97.74% accuracy.
