...
```{r}
# library(MASS)
LDA <- lda(class ~ u + g + r + i + z + redshift, data = Space_DF) # The main command for LDA
LDA
...
LDA.fit <- lda(class ~ u + g + r + i + z + redshift, CV = TRUE, data = Space_DF)
...
```{r}
LDA.training <- lda(class ~ u + g + r + i + z + redshift, data = S.Train) # Only used the training set
...
for(i in 1:10){
  ...
  lda_kf <- lda(class ~ u + g + r + i + z + redshift, data = train_kf )
...
```{r}
QDA <- qda(class ~ u + g + r + i + z + redshift, data = Space_DF) # The main command for QDA
QDA
```
...
QDA.fit <- qda(class ~ u + g + r + i + z + redshift, CV = TRUE, data = Space_DF)
...
```{r}
QDA.training <- qda(class ~ u + g + r + i + z + redshift, data = S.Train) # Only used the training set
...
for(i in 1:10){
  train_kf <- Space_DF[fold != i,]
  test_kf <- Space_DF[fold == i,]
  qda_kf <- qda(class ~ u + g + r + i + z + redshift, data = train_kf )
...
```{r}
# cross-validation
predictors <- c("u", "g", "r", "i", "z", "redshift")
