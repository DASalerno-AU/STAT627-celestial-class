...
mean(S.Test$class == LDA.pred$class) # 94.35%: Correct classification rate = proportion of correctly classified counts.
```
...
mean(pred_rate) # 94.53% for K-fold 10 validation of LDA
```
...
mean(S.Test$class == QDA.pred$class) # 94.91%: Correct classification rate = proportion of correctly classified counts.
```
...
mean(pred_rate) # 94.77% for K-fold 10 validation of QDA
```
...
mean(S.Test.PC$class == LDA.pred.PC$class) # 82.82%. It is quite lower than original 94.35%, correct classification rate.
```
...
mean(pred_rate) # 82.71%, It is quite lower than original 94.53% for K-fold 10 validation of LDA
```
...
mean(S.Test.PC$class == QDA.pred.PC$class) # 94.47%, it is almost the same as the original 94.91%.
```
...
mean(pred_rate) # 94.55%. It is almost the same as the original 94.77%
```
