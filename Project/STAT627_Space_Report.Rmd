...
CV.results[2,1] <- mean(S.Test$class == LDA.pred$class) # 94.35%: Correct classification rate = proportion of correctly classified counts.
```
...
CV.results[2,2] <- mean(pred_rate) # 94.53% for K-fold 10 validation of LDA
```
...
CV.results[3,1] <- mean(S.Test$class == QDA.pred$class) # 94.91%: Correct classification rate = proportion of correctly classified counts.
```
...
CV.results[3,2] <- mean(pred_rate) # 94.77% for K-fold 10 validation of QDA
```
...
CV.results.PC[2,1] <- mean(S.Test.PC$class == LDA.pred.PC$class) # 82.82%. It is quite lower than original 94.35%, correct classification rate.
```
...
CV.results.PC[2,2] <- mean(pred_rate) # 82.71%, It is quite lower than original 94.53% for K-fold 10 validation of LDA
```
...
CV.results.PCA[3,1] <- mean(S.Test.PC$class == QDA.pred.PC$class) # 94.47%, it is almost the same as the original 94.91%.
```
...
CV.results.PCA[3,2] <- mean(pred_rate) # 94.55%. It is almost the same as the original 94.77%
```
...
CV.results[4,2] <- mean(class.rates)
```
...
CV.results.PC[4,1] <- VS$class.rate
VS$confusion.matrix
```
...
CV.results.PC[4,2] <- mean(class.rates)
```
